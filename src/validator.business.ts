/*
This code is based on the implementation of the credit card validator of the 'validatorjs' library:
https://github.com/validatorjs/validator.js/blob/master/src/lib/isCreditCard.js

You can find more information about the library in its github repository:
...
export const isCreditCard = (str: string) => {
  ...
  for (let i = sanitized.length - 1; i >= 0; i--) {
    ...
    if (shouldDouble) {
      tmpNum *= 2;
      tmpNum >= 10 ? (sum += (tmpNum % 10) + 1) : (sum += tmpNum);
    } else {
      sum += tmpNum;
    }
